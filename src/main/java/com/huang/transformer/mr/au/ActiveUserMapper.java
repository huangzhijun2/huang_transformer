...
public class ActiveUserMapper extends TableMapper<StatsUserDimension, TimeOutputValue> {
    ...
    private BrowserDimension defautBrowser = new BrowserDimension("", "");//默认的browser对象
    ...
    protected void map(ImmutableBytesWritable key, Result value, Context context) throws IOException, InterruptedException {
        ...
        //进行platform的构建
        List<PlatformDimension> platforms = PlatformDimension.buildList(platform);//platfor创建
        //获取browser name 和 browser version
        ...
        for (PlatformDimension pf : platforms) {
            this.outputKey.setBrowser(defautBrowser);//进行覆盖操作
